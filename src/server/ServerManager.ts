	...
	private _loadSettings(): void {
			...
			this.app.use(
				Cors({
					origin: (origin, callback) => {
						if (
							this.settings.cors.origin.includes(origin) ||
							origin === '*'
						) {
							callback(null, true);
						} else {
		...
		if (this.settings.controllers) {
			for (const controller_target of this.settings.controllers) {
				const controller_storage = Storage.getControllerByTarget(
					controller_target,
				);
				...
				for (const route of routes) {
					(controller_router as any)[
						...
						(
							...
							controller_target.prototype[route.original_name](
								request,
								response,
							),
					);
	...
	listen(): Promise<void> {
		return new Promise((resolve) => {
			this.app.listen(this.settings.port, this.settings.host, () => {
				...
				LogsManager.success(
					`Server initialized on ${this.settings.host}:${this.settings.port}`,
				);
			});
		});
	}
